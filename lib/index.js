/* lib/index.js
 * This is the main file for node-upb.
...
//    }
  ...
  upb.validCommands = [
    ...
    31, '31', 'storeState',
    80, '80', 'ackResponse',
    85, '85', 'setupTimeReport',
    86, '86', 'deviceStateReport',
    87, '87', 'deviceStatusReport',
    90, '90', 'registerValuesReport',
    91, '91', 'RAMvaluesReport',
    92, '92', 'rawDataReport',
  ...
  upb.generate = function (command) { // generate(commandInputAsJSON, function(commandJSON, err){});
    return new Promise(function generateReturn(resolve) {
      if (!command.network) { throw new Error ('No network id specified!'); }
      if (!command.id) { throw new Error ('No link or device id specified!'); }
      if (!command.type) { throw new Error ('No id type specified!'); }
      if (!command.cmd) { throw new Error ('No command specified!'); }
      if (command.powerlineRepeater === 0) { command.powerlineRepeater = false; }
      if (command.powerlineRepeater === true) { command.powerlineRepeater = 1; }
      if (command.powerlineRepeater && !(command.powerlineRepeater === 1 || command.powerlineRepeater === 2 || command.powerlineRepeater === 4 || command.powerlineRepeater === false || command.powerlineRepeater === 0)) { throw new Error ('Power line repeater argument only accepts 1, 2, 4, or false!'); }
      if (command.level > 100 || command.level < 0) { throw new Error ('Level can only be values 0 through 100!'); }
      if (command.sendTime && command.sendTime > command.sendx) { throw new Error ('Number of time sent (sendTime) cannot be greater than total number of times sent (sendx)!'); }
      command.source = command.source || 255;
      ...
      else if (command.cmd === 143 || command.cmd === '143' || command.cmd === 'deviceSignatureReport') {command.msg = 143; command.cmd = 'deviceSignatureReport';}
      else {
        throw new Error ('Invalid command specified! Please us the correct command name or number.');
      }
      command.hex.msg = parseInt(command.msg, 16).toString(16);
      
      if ((command.cmd === 'goto' || command.cmd === 'fadeStart' || command.cmd === 'deviceStateReport') && !command.level) { throw new Error ('No level specified! A level must be specified with goto, fadeStart, and deviceStateReport.'); }
      if (command.cmd === 'blink' && !command.blinkRate) { throw new Error ('No blink rate specified!'); }
      if (command.cmd === 'toggle' && !command.toggleCount) { throw new Error ('No toggle count specified!'); }
      ...
      command.generated += upb.util.pad(command.hex.network, 2, 0);
      command.generated += upb.util.pad(command.hex.id, 2, 0);
      command.generated += upb.util.pad(command.hex.source, 2, 0);
      command.generated += upb.util.pad(command.msg, 2, 0);
      if (command.hex.level) { command.generated += upb.util.pad(command.hex.level, 2, 0); }
      if (command.hex.rate) { command.generated += upb.util.pad(command.hex.rate, 2, 0); }
      if (command.hex.blinkRate) { command.generated += upb.util.pad(command.hex.blinkRate, 2, 0); }
      if (command.hex.toggleCount) { command.generated += upb.util.pad(command.hex.toggleCount, 2, 0); }
      if (command.hex.toggleRate) { command.generated += upb.util.pad(command.hex.toggleRate, 2, 0); }
      if (command.hex.channel) { command.generated += upb.util.pad(command.hex.channel, 2, 0); }
      
      command.checksum = upb.generateChecksum(command);
      command.generated += upb.util.pad(command.checksum, 2, 0);
  ...
  upb.decode = function (commandGenerated) { // decode(commandInputAsStr, function(commandDecodedJSON, err){});
    return new Promise(function decodeReturn(resolve) {
      ...
      else { throw new Error ('Invalid command! Powerline repeater argument can only be 1, 2, or 4 which translates into 2, 4, or 6.'); }
      
      command.words = command.ctrlWord.nibble2;
      if (command.generated.length/2 !== command.words) {
        throw new Error ('Invalid command! Word count doesn\'t add up.');
      }
      ...
      if (command.ctrlWord.nibble3 > 7) { throw new Error ('Invalid command! Any number greater than seven cannot be an Acknowledge Pulse, ID Pulse, or Acknowledge Message.'); }
      ...
      else { throw new Error ('Invalid command! Send time is greater than total sent times.'); }
      ...
      command.msg = Number(command.generated.substring(10,12).toString(16));
      ...
      else { throw new Error('An unknown command was specified!'); }
      command.hex.msg = parseInt(command.msg, 16).toString(16);
      ...
      if ((command.cmd === 'goto' || command.cmd === 'fadeStart') && !command.level) { throw new Error ('Invalid command! No level specified. A level must be specified with goto and fadeStart.'); }
      if (command.cmd === 'blink' && !command.blinkRate) { throw new Error ('No blink rate specified. Blink rate must be specified with the blink command.'); }
      if (command.cmd === 'toggle' && !command.toggleCount) { throw new Error ('Invalid command! No toggle count specified. Toggle count must be specified with the toggle command.'); }
