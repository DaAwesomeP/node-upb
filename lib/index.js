...
//    }
  ...
  upb.generate = function (command) { // generate(commandInputAsJSON, function(commandJSON, err){});
    return new Promise(function generateReturn(resolve, reject) {
      if (!command.network) { reject('No network id specified!'); }
      if (!command.id) { reject('No link or device id specified!'); }
      if (!command.type) { reject('No id type specified!'); }
      if (!command.cmd) { reject('No command specified!'); }
      ...
      if (command.powerlineRepeater && !(command.powerlineRepeater === 1 || command.powerlineRepeater === 2 || command.powerlineRepeater === 4 || command.powerlineRepeater === false || command.powerlineRepeater === 0)) { reject('Power line repeater argument only accepts 1, 2, 4, or false!'); }
      if (command.level > 100 || command.level < 0) { reject('Level can only be values 0 through 100!'); }
      if (command.sendTime && command.sendTime > command.sendx) { reject('Number of time sent (sendTime) cannot be greater than total number of times sent (sendx)!'); }
      ...
      if (command.cmd === 20 || command.cmd === '20' || command.cmd === 'activate') {command.cmd = 'activate'; command.msg = 20;}
        ...
        reject('Invalid command specified! Please us the correct command name or number.');
      }
      command.hex.msg = parseInt(command.msg, 16).toString(16);
      
      if ((command.cmd === 'goto' || command.cmd === 'fadeStart' || command.cmd === 'deviceStateReport') && !command.level) { reject('No level specified! A level must be specified with goto, fadeStart, and deviceStateReport.'); }
      if (command.cmd === 'blink' && !command.blinkRate) { reject('No blink rate specified!'); }
      if (command.cmd === 'toggle' && !command.toggleCount) { reject('No toggle count specified!'); }
  ...
  upb.decode = function (commandGenerated) { // decode(commandInputAsStr, function(commandDecodedJSON, err){});
    return new Promise(function decodeReturn(resolve, reject) {
      ...
      else { reject('Invalid command! Powerline repeater argument can only be 1, 2, or 4 which translates into 2, 4, or 6.'); }
      
      command.words = command.ctrlWord.nibble2;
      if (command.generated.length/2 !== command.words) {
        reject('Invalid command! Word count doesn\'t add up.');
      }
      ...
      if (command.ctrlWord.nibble3 > 7) { reject('Invalid command! Any number greater than seven cannot be an Acknowledge Pulse, ID Pulse, or Acknowledge Message.'); }
      ...
      else { reject('Invalid command! Send time is greater than total sent times.'); }
      ...
      if ((command.cmd === 'goto' || command.cmd === 'fadeStart') && !command.level) { reject('Invalid command! No level specified. A level must be specified with goto and fadeStart.'); }
      if (command.cmd === 'blink' && !command.blinkRate) { reject('No blink rate specified. Blink rate must be specified with the blink command.'); }
      if (command.cmd === 'toggle' && !command.toggleCount) { reject('Invalid command! No toggle count specified. Toggle count must be specified with the toggle command.'); }
